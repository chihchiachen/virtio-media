 ...
 *
 * This is enabled by default as guest applications expect timestamps to match
 * the guest system's CLOCK_MONOTONIC timebase.
 */
bool virtio_media_override_timestamps = true;
module_param_named(override_timestamps, virtio_media_override_timestamps, bool,
		   0660);
				 ...
				 struct virtio_media_event_dqbuf *dqbuf_evt)
{
	 ...
	 * upon buffer dequeue.
	 */
	if (virtio_media_override_timestamps &&
